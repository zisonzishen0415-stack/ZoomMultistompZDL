...
> 2026-08-16 的 G1on 实机失败（无 UI、bypass、或切换死机）根因：
> `effectTypeImageInfo +0x20` 被 linker 写成总参数数（Reverson=9），
> 而该字段是**每页可见旋钮槽数**（stock 9 参数效果均写 3，GEQ 类写 6/7）。
> 已修复并重建（详见 `docs/LOADER-SAFETY.md` §6 冻结原因表）。
> 复测顺序：K9Probe.ZDL（合成 handler 旋钮 4..9）→ Reverson.ZDL。
